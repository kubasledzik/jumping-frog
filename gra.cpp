...
void init_frog(GameConfig* game_config, Frog* frog) {
    ...
    frog->direction = 'U';                  // Początkowy kierunek żaby (pionowo)
    frog->moves = 0;
...
void change_car_position(Car *car, GameConfig *game_config, int roads_pos[], int cars_on_lane[], int *free_lanes){
    int lane = rand() % game_config->road_lanes;
    if(*free_lanes == 0){
        car->y = roads_pos[lane * 2] + 1;
        cars_on_lane[lane]++;
    }
    else {
        for(int i = 0; i < game_config->road_lanes; i++){
            if(cars_on_lane[i] == 0){
                car->y = roads_pos[i * 2] + 1;
                cars_on_lane[i]++;
                (*free_lanes)--;
                break;
            }
...
void update_car_pos(GameConfig *game_config, Car *car, int roads_pos[], int cars_on_lane[], int *free_lanes){ //there's 50% chance for car to turn around or to change lane
    car->x += car->direction;
...
void cars_move(GameConfig *game_config, Car* cars, int roads_pos[], int cars_on_lane[], int *free_lanes){
    for(int i = 0; i < game_config->car_number; i++){
        if((clock() - cars[i].last_move_time) * 1000 / CLOCKS_PER_SEC >= cars[i].delay){
            int temp = 0;
            for(int j = 0; j < game_config->road_lanes; j++){
                if(roads_pos[j] == cars[i].y){
                    temp = j;
                }
            }
            cars_on_lane[temp]--;
            if(cars_on_lane[temp] == 0){
                (*free_lanes)++;
            }
...
int main() {
    ...
    clock_t start_time = clock(); // Początek gry
    clock_t last_move_time = clock();
    WINDOW* game_window = newwin(game_config->height + 2, game_config->width + 2, 0, 0); // Tworzy okno o wymiarach planszy + ramka
